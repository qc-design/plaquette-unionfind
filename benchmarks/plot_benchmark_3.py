...
for line in data:
    ...
    name = line[0]
    x = float(line[2]) # probability
    n_rounds = int(line[1])-1
    y = float(line[3]) * 1000 / float(n_rounds)  # Scale y-axis to milliseconds
...
plt.xlabel('Error Probability', fontsize=12, weight='bold')  # Update x-axis label
plt.ylabel('Time per Round (ms)', fontsize=12, weight='bold')  # Update y-axis label
